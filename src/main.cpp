...
#if defined(NDEBUG)
# error "Bbtcoin cannot be compiled without assertions."
#endif
...
const string strMessageMagic = "Bbtcoin Signed Message:\n";
                        ...
                        bool* pfMissingInputs)
{
    ...
    // Bbtcoin: coinstake is also only valid in a block, not as a loose transaction
    if (tx.IsCoinStake())
...
// Bbtcoin: find block wanted by given orphan block
...
// Bbtcoin: find last block index up to pindex
                                 ...
                                 const CBlockIndex* pindexBlock, bool fBlock, bool fMiner, unsigned int flags)
{
    ...
    // fMiner is true when called from the internal Bbtcoin miner
    ...
    if (!IsCoinBase())
    {
        ...
        for (unsigned int i = 0; i < vin.size(); i++)
        {
            ...
            // Bbtcoin: check transaction timestamp
...
bool CBlock::DisconnectBlock(CTxDB& txdb, CBlockIndex* pindex)
{
    ...
    // Bbtcoin: clean up wallet after disconnecting coinstake
...
bool CBlock::ConnectBlock(CTxDB& txdb, CBlockIndex* pindex, bool fJustCheck)
{
    ...
    if (IsProofOfStake())
    {
        // Bbtcoin: coin stake tx earns reward instead of paying fee
        uint64_t nCoinAge;
    ...
    // Bbtcoin: track money supply and mint amount info
...
// Bbtcoin: total coin age spent in transaction, in the unit of coin-days.
...
bool CBlock::AddToBlockIndex(unsigned int nFile, unsigned int nBlockPos, const uint256& hashProof)
{
    ...
    // Bbtcoin: compute chain trust score
    pindexNew->nChainTrust = (pindexNew->pprev ? pindexNew->pprev->nChainTrust : 0) + pindexNew->GetBlockTrust();

    // Bbtcoin: compute stake entropy bit for stake modifier
    ...
    // Bbtcoin: compute stake modifier
...
bool ProcessBlock(CNode* pfrom, CBlock* pblock)
{
    ...
    if (!mapBlockIndex.count(pblock->hashPrevBlock))
    {
        ...
        if (pfrom) {
            ...
            // Bbtcoin: getblocks may not obtain the ancestor block rejected
...
#ifdef ENABLE_WALLET
// Bbtcoin: attempt to generate suitable proof-of-stake
